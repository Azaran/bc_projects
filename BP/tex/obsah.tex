...
k~tomu urèeny. 
 ...
 Toho lze dosáhnout ¹ifrováním informací. V~dne¹ní dobì exituje nezpoèet mo¾ností jak
informace ¹ifrovat. Kvalitu zabezpeèení dat znaènì vlivòuje výbìr pou¾itého ¹ifrovacího algoritmu.
Ov¹em zvolení správného hesla má vliv je¹tì vìt¹í. Pokud pou¾ijete slabou ¹ifru a silné heslo, je
¹ance, ¾e se k~Va¹im datù dostane neoprávnìná osoba je o~dost men¹í, ne¾ kdy¾ to udìláte naopak.
...
typù souborových archivù jsou formáty .ZIP a .7z.
 ...
 V~této práci se pøevá¾nì zabývám analýzou pou¾ívaných metod zabezpeèení a ¹ifrovacích
...
\section{Triple Data Encryption Algorithm (TDEA, 3DES)}
...
\begin{itemize}
    ...
    \item Posledním re¾ímem je pou¾ití jednoho 56-bitového klíèe a tedy K1 = K2 = K3.
\end{itemize}
...
nebo dvoudimenziálních polí bajtù. V~metodì AES je pou¾ito dvoudimenzionální pole skládající se ze
ètyø øádkù obsahujících ètyøi bajty. Toto dvou dimenzionální pole je nazýváno {\it State}.

Na zaèátku ¹ifrování se nakopírují data ze vstupu do pole {\it State} a nainicializují se
...
\section{SHA-1}
Slou¾í pro he¹ování\footnote{Vytvoøení otisku zprávy s~konstantní výstupní velikostí pro jakkoliv
velké vstupní zprávy.} zpráv s~maximální délkou $2^{64}-1$ bitù. Jako výstup produkuje 160 bitovou
...
pøípadù potøebuje krat¹í výpoèetní èas (popsána v~Algoritmu \ref{alg:SHA}). Druhá se spí¹e hodí
pro systémy s~omezenými zdroji a které nevy¾adují co nejrychlej¹í zpracování.


\section{SHA-256}
Nástupce SHA-1 mající stejná omezení pro vstupní zprávu jako jeho pøedchùdce, av¹ak li¹í se v~délce
...
\begin{algorithm}[ht]
    ...
    \caption{Princip SHA funkce}\label{alg:SHA}
\end{algorithm}
...
výpoèetní jednotky (GPU). Existence tìchto typù zaøízení ústí v~potøebu mít prostøedky jak s~tìmito
...
jedno nebo více výpoèetních jednotek, které jsou dále dìleny na jeden nebo více výpoèetních prvkù.
...
pamìti pou¾ívané v~platformì vytvoøené tímto frameworkem.
 ...
 Pro ka¾dou aplikaci je tøeba pøesnì definovat jak budou jednotlivé pamìti v~pamìti vypadat.
...
implementovány tøi typ generátorù:
\begin{itemize}
...
\subsection{Crackery}
Druhou èástí jsou samotné crackery, které se dìlíme podle toho, kdo provádí generování hesel a
...
prodlevu výpoètù.
 ...
 V~obou pøípadech je nutné pøed zahájením jakýchkoliv operací GPU inicializovat OpenCL
systém. Tedy vytvoøit kontext, fronty pøíkazù, zajistit naètení a pøelo¾ení po¾adovaného kernelu a
a¾ poté nahrát nebo vygenerovat data do/na GPU.
...
6.3.3 z~roku 2012, pøijat Mezinárodní orgranizací pro normalizaci (ISO) a Mezinárodní
...
Archivy jsou takové soubory, jen¾ v~sobì nesou dal¹í soubory. Dá se tedy øíct, ¾e se jedná
o~jakési \uv{schránky} do nich¾ lze vkládat, ale i vybírat, urèité soubory rùzných typù.
...
lokální hlavièky nebo z~jeho hlavièky centrálního adresáøe a konkétnì z~prvního a sedmého bitu
...
\begin{itemize}
	...
	¹ifrování}, hlavièka ¹ifrování se negeneruje, ale pøidávájí se informace o~¹ifrování do
        hlavièky centrálního adresáøe a generuje se záznam s~hlavièkou pro de¹ifrování, který se
        tváøí jako souèást dat souboru a nachází se na samém zaèátku dat.
\end{itemize}
...
\begin{itemize}
    ...
    \item Tøetí èást obsahuje zpracované :ormace slou¾ící jako podpùrná data pro hlavièku a její
	polo¾ky.
	...
	Identifikátory zaèínají hodnotou 0x00 a konèí 0x19. Máme tedy k~dispozici dvacetpìt
	polo¾ek s~rùznou velikostí, strukturou a polo¾kami. Jedinými povinnými údaji v~hlavièce
	jsou polo¾ky znaèící zaèátek hlavièky (0x01) a konec hlavièky (0x00). V¹echny ostatní
	polo¾ky jsou pøípadnì umístìny mezi nì a zaèínají pøíslu¹ným
	identifikátorem~\cite{Pavlov:2010}.  
\end{itemize}
\begin{figure}[ht]
    \begin{center}
	\scalebox{0.35}{
...
pou¾ívány a jaké soubory obsahují.
 ...
 Formát .ZIP, ale pøekonává .7z mo¾ností pou¾ití elektronických podpisù (certifikát typu
...
\section{Modul ZIP}
Jako první si nastíníme modul ZIP. Pokud se divíte proè se zde bavíme o návrhu modulu ZIP,
kdy¾ jak u¾ bylo vý¹e øeèeno je tento modul pro Wrathion ji¾ vytvoøen, tak se divíte správnì. V
pøípadì tohoto modulu se bude zabívat roz¹íøením funkcionality a ne vytvoøením zcela nového
modulu.

 Momentální verze ZIP modulu podporuje pouze staré ¹ifrování PKWARE a ¹ifrování AES. Podpora
¹ifrování AES je v¹ak relativní. Nemù¾eme øíct, ¾e bychom mìli 100\% funkcionalitu a byli
schopní zjistit hesla pro v¹echny soubory za¹ifrované metodou AES. Momentálnì podporujeme pouze
speciální verzi AES, jen¾ si vytvoøily vývojáøi nástroje WinZip. Ti si vytvoøili vlastní oøezanou
AES hlavièku a oznaèili soubor vlastní metodou. Existují ov¹em nástroje jako SecureZIP od firmy
PKWARE umo¾òující také za¹ifrovat obsah archivù pomocí ¹ifrovací metody AES, s ní¾ si souèasný
modul i pøes podporu zji¹tìní hesla pro AES neporadí.

 Dal¹ím ¹ifrovací metodou, kterou nabízí nástroj SecureZIP je 3DES. S ním si momentální verze
neporadí vùbec a tak je tedy vhodné tuto funkcionalitu doplnit, abychom sní¾ili poèet ZIP archívù,
jen¾ tento modul nezvládne zpracovat.
...
\begin{enumerate}
    ...
    \item Pøeètení {\it General purpose bit flag} -- hodnotu pøíznakù (flagù) získáme z
	nìkteré z hlavièek\footnote{Lokální hlavièka souboru nebo hlavièka souboru v centrálním
    ...
    \item Analýza pøíznakù -- zji¹»ujeme zda jsou nastaven první bity pøíznakù na hodnotu 1. Pokud
	nalezneme ¹ifrování u jednoho souboru pøedpokládáme, ¾e v¹echny ostatní soubory jsou
	takté¾ ¹ifrované a ¾e byla pou¾ita stejná ¹ifrovací metoda.
    \item Pøeètení {\it Compression method} -- z nìkteré z hlavièek\footnotemark[\value{footnote}]
	souboru. Tato polo¾ka nás zajímí pouze pokud byly bity jedna a sedm pøíznakù nastaveny na
	hodnotu 1. Pakli¾e byly èislo uvedené v této polo¾ce nám dovoluje zjisti zda byla pou¾ita
	¹ifrovací metoda WinZip AES (hodnota 99). Pou¾ití jiné hodnoty indikuje pou¾ití nìkteré z
	dal¹ích ¹ifrovacích metod definovaných ve specifikaci formátu .ZIP~\cite{PKWARE:2014}.  
...
bude mo¾né realizovat za vyu¾ití CPU nebo GPU. Pro obì tyto zaøízení je nutné vytvoøit samostatný
...
v OpenCL pro implementování kernelu, který pobì¾í na pracovních jednotkách. Av¹ak v obouch
...
\section{Modul 7z}
V tomto pøípadì se jedná o úplnì nový modul. Bude tedy potøeba nadefinovat v¹echny potøebné
struktuty potøebné k provozu modulu v rámci nástroje Wrathion. Také bude potøeba implementovat
metody ¹ifrování, he¹ování, odvozování hesel a jiné pou¾ívané formátem .7z. 

\subsubsection{Zji¹tìní informací z archivu}
Archiv formátu .7z v sobì, stejnì tak jako .ZIP archiv, nese informace nutné pro umo¾nìní
de¹ifrování souboru. V tomto pøípadì je ov¹em o nìco slo¾itìj¹í je získat. Struktura 7z archivu
je hodnì komplexní a promìnlivá a struktura hlavièek pøipomíná databázi. Pro detailní porozumnìní
struktuøe je tedy potøeba nahlídnout do souboru 7zFormat.txti~\cite{Pavlov:2010}. K
informacím se dostaneme následujícím zjednodu¹eným teoretickým postupem:
\begin{enumerate}
    \item Ovìøíme signaturu -- jedná se o první 6 bajtù souboru, které musí odpovídat hodnotì v
	sekci \ref{ssec:7z_struct} a 
    \item Získáme offset a velikost dal¹í hlavièky -- pøeskoèíme na pozici 6 bajtù za signaturou a
	získáme offset jen¾ nám øekne, kde zaèíná dal¹í hlavièka. Po pøeètení hodnoty za ní
	následující získáme délku této hlavièky.
    \item Pøejdeme na dal¹í hlavièku -- musíme se podívat na signaturu hlavièky, pokud je rovna
	0x17 tak musíme projít následující polo¾ky.
    \item Detekce ¹ifrování hlavièky -- hledání provádíme v hlavièce {\it Folders} se
	signaturou 0x0B, prvnì hledáme hodnotu [0x06, 0xF1, 0x07, 0x01] øíkající, ¾e jsou
	data hlavièky ¹ifrována	metodou 7zAES.
    \item Získání dat pro ¹ifrování -- v pøípadì nálezu èteme data inicializaèního vektoru a poèet
	iterací, kterým byl proveden výpoèet klíèe pro ¹ifrování (defaultnì 19).
    \item De¹ifrování hlavièky -- pro de¹ifrování podle algoritmu~\ref{alg:7z_desifr}.
    \item Detekce hlavièky pro dekompresy -- hledáme ve stejné sekci jako v bodì 5. Zde v¹ak
	pátrame po hodnotì [0x03, 0x01, 0x01] znaèící pou¾ití LZMA pro kompresy.
    \item Provedeme dekompresy hlavièky -- pomocí algoritmu LZMA.
    \item Vypoèítáme CRC dat po dekompresy.
    \item Porovnáme vypoètené CRC s odpovídajícím CRC ulo¾eným v hlavièce {\it Digest}, pokud ne
	vracíme se na krok 6.
    \item Pro zji¹tìní hesla k souborùm se podíváme do dekomprimovaných dat a postupujeme stejnì
	od kroku 4.
\end{enumerate}
...
ve vet¹inì pøípadù ani nestojí za námahu. Dùvodem je nízký poètu dostupných nástrojù schopných tyto
hodnoty nastavit, platí hlavnì pro formát ZIP.

 Struktury jednotlivých formátù jsou popsány tak, aby bylo mo¾né chybìjící nebo nejasné informace
mo¾né dohledat bez vìt¹ích problémù ve specifikaci formátu. Zde je nutno podotknout, ¾e jak ji¾
bylo zmínìno struktura 7z souboru je hodnì komplexní a promìnná. Bohu¾el dokumentace tohoto
formátu znaènì nedostaèující a je tedy nutné strávit hodnì èasu studiem struktury, zdrojových
souborù a fóra vývojáøe.

 Návrh roz¹íøení modulu pro ZIP by mìl z vìt¹iny odpovídat tomu jak bude roz¹íøení vypadat po jeho
implementaci. Také na základì prùzkumu v tomto odvìdví by roz¹íøený modul mìl být napøed pøed
konkurencí. Nìkteré známé nástroje toti¾ mají podporu pouze pro WinZip AES a PKWARE ¹ifrování
(John the Ripper) nebo nemají zatím podporu ZIP formátu vùbec (oclHashcat).  

 U modulu 7z se jedná spí¹ o teoretický nástin jak by mohl modul fungovat a jak by
mohly jednotlivé èásti algoritmu vypadat. Nìkteré dùle¾ité informace toti¾ nejsou nikde detailnì
popsány. Návrh modulu bude tedy potøeba pravdìpodobnì modifikovat na základì informací zji¹tìných
bìhem implementace jednotlivých èástí modulu.
