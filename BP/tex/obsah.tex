...
k~tomu urèeny. 
 ...
 Toho lze dosáhnout ¹ifrováním informací. V~dne¹ní dobì máme nezpoèet mo¾ností jak ¹ifrovat
...
se k~Va¹im datù dostane je o~dost men¹í ne¾ kdy¾ to udìláte naopak. 
 ...
 Èasto chceme sdílet nebo zabezpeèit více jak jeden soubor s~informacemi. Obecnì se pro
...
dat. Metoda pracuje s~64-bitovým klíèem ov¹em pro ¹ifrování je pou¾ito pouze 56 bitý zbylé bity
...
\begin{itemize}
    ...
    \item Posledním re¾ímem je pou¾ití jednoho 56-bitový klíèe a tedy K1 = K2 = K3.
\end{itemize}
V~první a druhý re¾im jsou jedinými mo¾nostmi schválenými standardem jako dostateèné. Tøetí re¾im je
pou¾it pouze pro kompatibilitu s~DES. Pokud se toti¾ podíváme na provádìné kroky tak zjistíme, ¾e
pokud jsou v¹echny klíèe identické je výstup TDEA identický s~výstupem DES pøi pou¾ití stejného
...
Jedná se o~symetrickou blokovou ¹ifru pou¾ívající 128-bitový datový blok a promìnnou délkou
¹ifrovacího hesla. Délka hesla mù¾e být 128, 192 nebo 256 bitù. To je reflektováno v~názvu metody
(AES-128, AES-192, AES-256). Metoda pracuje s~daty po bajtech, ktere pripadne organizuje do poli
...
pøípadù potøebuje krat¹í výpoèetní èas (popsána v~Algoritmu \ref{alg:SHA}). Druhá se spí¹e hodí pro systémy s~omezenými zdroji a které
nevy¾adují co nejrychlej¹í zpracování.
...
výstupní hodnoty, kde v~tomto pøípadì má velikost 256-bitù. To poskytuje podstatnì víc mo¾ných
...
work-group}).
 ...
 Hostitel má za úkol spravovat kontext aplikace a na základì nìho nastavit prostøedí, ve
...
První èástí Wrathionu, kterou si rozebereme jsou generátory hesel pro útoky. Jednáse se esenciální
...
\begin{itemize}
	...
	hesly
\end{itemize}
Nejèastìji pou¾ívaným generátor je brute-force, který je ov¹em výpoèetnì nároèný. Jedná se
...
Zde se standardnì spustí ji¾ pøedkompilovaný cracker napsaný v~C++ (viz. Obr.\ref{CPU}). Není zde ¾ádný vìt¹í problém
...
Cracker mù¾e pracovat ve dvouch re¾imech (viz. Obr. \ref{CPUGPU}). Prvním je, ¾e hesla generujeme na CPU a pak je posíláme
...
21320-1:2015~\cite{ISOIEC:2015}.
 ...
 Formát podporuje velké mno¾ství rùzný komprimaèních algoritmù: Store(bez komprese),
...
\end{itemize}
Nastává zde ale problém. ®ádný z~a» u¾ volnì dostupných nebo komerèních nástrojù
...
Archívy jsou takové soubory jen¾ v~sobì nesou dal¹í soubory. Dá se tedy øíct, ¾e jedná
o~jakési \uv{schránky} do nich¾ lze vkládat, ale i vybírat, urèité soubory rùzných typù.
U~formátu .ZIP lze do archívù navíc ukládat i adresáøe a tak ukládat
...
\end{figure}
 ...
 Obsah souboru archívu lze pro lep¹í orientaci rozdìlit na èást obsahující definice a data
...
\end{itemize}
Od verze formátu 6.2 jsou pøed hlavièku centrálního adresáøe dal¹í polo¾ky a to hlavièka
...
\subsubsection{Øídící struktury definující ¹ifrování}
 ...
 Doteï jsme se bavili pouze o~základní struktuøe archívu. Nás ale spí¹e zajímají struktury
...
polo¾ky {\it General Purpose Bit Flag}. Nastavení prvního bitu indikuje, ¾e soubor ¹ifrován.
\begin{itemize}
    ...
    \item Pokud je nastaven i sedmý bit indikující pou¾ití takzvaného \uv{silného ¹ifrování}.
        V~tomto pøípadì se hlaèka ¹ifrování negeneruje, ale pøidávájí se informace o~¹ifrování do
...
klíèe pro ¹ifrování a pole pøíznakù {\it Flags} definující, zda je archív ¹ifrovaný, jaká je pou¾ita
kompresní metoda, jaká Dco je vy¾adováno prode¹ifrování, jestli je
vy¾adováno heslo nebo pouze certifikát nebo jedna ze obou uvedených, tedy heslo nebo certifikát,
pøípadnì dal¹í mo¾nosti v~souvislosti s~pou¾itým certifikátem.

 Záznam pro de¹ifrování obsahuje podobné informace jako {\it Extra Field} roz¹íøené o~délku
...
k~vytvoøení souborových archívù podobnì jako .ZIP. Formát se proslavil hlavnì svojí otevøeností a
...
\end{itemize}
Dal¹ími výhodami formátu je podpora komprese velkých souborù, názvy souborù v~Unicode
...
a ¹ifrování hlavièek archívù a dal¹í.
 ...
 Standardnì pou¾itou ¹ifrovací metodou je AES-256 vy¾adující 256-bitovým ¹iforvacím heslem.
...
\subsection{Struktura souboru}
Neprázdný soubor tohoto formátu má ètyøi èásti, které v~nìm musí být obsazeny viz.
...
\begin{itemize}
	...
	polo¾ky.
    \item Poslední ètvrtá èástí je hlavièka {\it 7zHeader}, její¾ zaèátek je definován
v~úvodní hlavièce. Tato hlavièka má promìnnou délku a strukturu. Je ji tedy potøeba procházet
	postupnì a zji¹»ovat, které polo¾ky jsou pøítomny a které ne. K~identifikaci jednotlivých
	polo¾ek slou¾í jednobajtový identifikátor zapsaný v~hexadecimální formì. Identifikátory
	zaèínají hodnotou 0x00 a konèí 0x19i. Máme tedy k~dispozici dvacetpìt polo¾ek s~rùznou
...
\section{Srovnání}
Pokud budeme srovnávat formáty z~pohledu komprese, zjistíme, ¾e má .7z je dle mìøení efektivnìj¹í
ne¾
