...
[0x06, 0xF1, 0x07, 0x01]~\cite{Pavlov:2015}. Zde je tøeba také nálezt data o pou¾ité kompresi.
Kompresní metoda LZMA má identifikátor [0x03, 0x01, 0x01]. Dal¹í dùle¾itou polo¾kou pro ovìøení hesla je CRC ¹ifrovaného souboru. 

Tento typ archivu bohu¾el neobsahuje ¾ádnou jinou metodu ovìøení hesla ne¾ pomocí spoèítání a
porovnání CRC hodnot. K tomu je nutné nejprvek celý soubor de¹ifrovat a dekomprimovat co¾ znaènì
pøidává na nároènosti celého ovìøovacího procesu.
...
\subsection{Zji¹tìní informací z~archivu}
...
roz¹íøení funkcionality modulu (detailní popis hlavièek a~hodnot v~nich je
...
\begin{enumerate}
    ...
    \item Pøeètení {\it General purpose bit flag} -- hodnotu pøíznakù získáme z lokální hlavièky
	prvního souboru.
...
\begin{algorithm}[ht]
    ...
    \AlgoDisplayBlockMarkers\SetAlgoNoLine%
    \DontPrintSemicolon
    \Input{Data získaná z~hlavièky pro de¹ifrování (IV, Erd, Dlen, Data)}
    \Output{Poslední vygenerované heslo}
    \Repeat{$ CRC != spocitejCRC32 (Data, Dlen-4) $}{
	heslo = generujHeslo ()\;
	odvozene\_heslo = odvodHeslo (SHA1(heslo))\;
	rd = desifruj (Erd, odvozene\_heslo, IV)\tcc*{Random hodnota pro ¹ifrování}
...
\begin{enumerate}
    \item Ovìøíme signaturu -- jedná se o~prvních 6 bajtù souboru, které musí odpovídat hodnotì
v~sekci \ref{ssec:7z_struct}. 
    \item Pøejdeme na pozici 6 bajtù za signaturou, kde	prvních 8 bajtù obsahuje pozici hlavièky.
	Na dal¹ích 8 bajtech je délka hlavièky.
    \item Pøejdeme na zaèátek hlavièky.
    \item Pøejdeme na záznam se signaturou 0x06, ze kterého získáme pozici dat k ní¾ pøièteme 32.
    \item Pøejdeme na záznam se signaturou 0x09, ze kterého získáme délku proudu dat.
    \item Pøejdeme na záznam se signaturou 0x0B, ve kteté hledáme hodnoty [0x06, 0xF1, 0x07, 0x01]
	a [0x03, 0x01, 0x01] znaèící 7zAES a LZMA.
    \item Získáme z nich data -- inicializaèní vektor, poèet iterací pou¾ití SHA256 (defaultnì
	19) pro 7zAES a inicializaèní data slovníkù pro LZMA.
    \item Pøejdeme na záznam se signaturou 0x0A, ze kterého získáme CRC dat souboru.
\end{enumerate}
Jakmile tato data získáme mù¾eme se pøesunout k hledání hesla. Tento proces, popsáný
Algoritmem~\ref{alg:7z_desifr}, lze plnì realizovat pouze na CPU. Formát 7z toti¾ mù¾e obsahovat
soubory o velké velikosti, které by se nemusely vejít do pamìti GPU. Musíme zde vzít toti¾ v
potaz, ¾e potøebujeme mít v pamìti prostor o velikosti souboru pro ka¾dou výpoèetní jednotku GPU,
na které pobì¾í kernel. Øe¹ení tohoto problému je popsáno v sekci \ref{ssec:7zcrackercpu}.
...
Tato kapitola se vìnuje popisu implementace pøidáné funkcionality modulu ZIP a nového modulu pro
7z do nástroje Wrathion. Jsou v ní také zmínìny pou¾ité knihovny a verze pou¾itých knihoven
implementujícíh de¹ifrovací a dekomprimaèní metody. Kapitola je rozdìlena do dvou èástí, ka¾dá
reprezentující jeden z formátù jimi¾ se tato práce zabývá.

\section{Implementace modulu ZIP}
Jak ji¾ bylo zmínìno tento modul bylo tøeba pouze obohatit o urèité nedostatky, které se týkaly
pøedev¹ím podpory metod ¹ifrování. Konktrétnì se jedná o metody ¹ifrování souborù AES a DES a také
o metodu umo¾òující ¹ifrování {\textit Central Directory} záznamu. Tyto metody jsou definované ve
standardu formátu ZIP vydaném firmou PKWARE v rámci APPNOTE.txt~\cite{PKWARE:2014} a pou¾ívány v
nástrojích z rodiny SecureZIP od PKWARE.

\subsection{ZIPFormat}
Tato tøída slou¾í ke ètení potøebných informací z archívu na vstupu. Získává tedy imformace o tom
jaká ¹ifrovací metoda byla pou¾ita na základì èeho¾ pak hledá dal¹í pro nás dùle¾itá data ve
struktuøe souboru. Data jsou ukladána do struktury $ZIPInitData$, která je pou¾ita pro
inicializaci nových instancí tøíd crackerù. 

 Tøída byla upravena tak, aby byla schopna detekovat vý¹e zmínìné metody a ulo¾it data potøebná pro
následné ovìøování hesel.

\subsection{ZIPStAESCrackerCPU}
Jedná se o tøídu s implementací pro ovìøení hesla metodou AES èistì na CPU. Tøída obsahuje funkci
pro ovìøení hesla $checkPassword()$ a funkce $derive()$, $derive\_key()$ a $sha1()$ pro vytvoøení
¹ifrovacích klíèù.

 Tøída vyu¾ívá implementace ¹ifrovací funkce AES pou¾ité v nástroji UnRAR, kde je
implementována jako tøída Rijndael pod licencí Public Domain. Datové typy pou¾íté v pøevzaté
tøídy byly pozmìny tak, aby odpovídaly typù pou¾ívaným v na¹em nástroji.
 
 Obì derivaèní funkce vytváøejí blok, který odpovídá funkci $CryptDeriveKey()$ z MS
CryptoAPI~\cite{CryptoAPI}. Funkce $sha1()$ je pøevzata z døíve implemetovaných èástí modulu ZIP.

 Funkce $checkPassword()$ implementuje algoritmus \ref{alg:zip_ver} jen¾ slou¾í k ovìøení hesla
získaného z generátoru. Algoritmus je odvozen algoritmu uvedeného v APPNOTE.txt zaèínajícího na
øádku 2720.

\subsection{ZIPStAESCrackerGPU}
Tato tøída obsahuje implementaci pro ovìøení hesla metodou AES na GPU. Tato tøída slou¾í pouze k
vytvoøení, inicializování a pøedání pamì»ových objektù (zásobníkù) OpenCL kernelu
$zip\_staes\_kernel$. To je realizováno ve funkci $initData()$. 

 K ovìøení hesla je realizováno GPU kernelem. Na CPU se po zji¹tìní nálezu provede pouze
kontrola voláním funkce $verifyPassword()$. Tato funkce vyu¾ívá objektu s typem tøídy
$ZIPStAESCrackerCPU$ a volání jeho funkce $checkPassword$, která je volá s kernelem oznaèeným
heslem na vstupu.
...
tøídy $ZIPStAESCrackerCPU$. Implementaènì je zde jeden znatelný rozdíl. Dynamické vytváøení Sbox
tabulek pro AES je nahrazeno konstantními tabulkami.
 
\subsection{ZIPTDESCrackerCPU}
Jedná se o tøídu obsahující pokus o implementaci metody 3DES pro CPU. Jedná se o tøídu velmi
podobnou tøídì $ZIPStAESCrackerCPU$. Jediným rozdílem je funkcí pro 3DES z knihovny OpenSSL
namísto funkcí pro AES.

 Tuto tøídu se mi bohu¾el nepovedlo zprovoznit. Pøíèinu problému s funkèností vidím v pøidávání
paritních bitù ke klíèi a inicializaènímu vektoru, jen¾ je knihovnou vy¾adováno. Dal¹ím
problémem s ladìním tohoto problému je, ¾e se mi nepovedlo nalést ani ¾ádný Open Source
archivaèní program, který by umìl takovýto soubor de¹ifrovat. To znaènì komplikuje proces ladìní,
kdy¾ není mo¾né porovnávat rozdíly v hodnotách v jednotlivých krocích.

\section{Implementace modulu SevenZ}
V tomto pøípadì bylo tøeba vytvoøit úplnì nový modul. K tomu bylo tøeba vytvoøit soubory {\textit
SevenZModule.cpp, SevenZFormat.cpp} a soubory pro CPU a GPU cracker. Soubory v sobì nesou
stejnojmenné tøídy implementující funkce vy¾adované pro správnou komunikaci s na¹ím nástrojem.

\subsection{SevenZFormat}
Podobnì jako tøída $ZIPFormat$ i tato slou¾í k získání a zpracování informací ulo¾ených v
archivu. Pro ulo¾ení byla vytvoøena struktura $SevenZInitData$ skádající se z polo¾ek základních
datových typù, ale také z dal¹ích vytvoøených struktur. Mezi nì jmenovitì patøí $SevenZCoder$,
$SevenZFolder$ a $SevenZPackInfoHdr$, které reflektují polo¾ky obsa¾ené ve struktuøe hlavièky
souboru. Tento pøístup k ulo¾ení dat byl zvolen na základì ji¾ zmínìného problému jím¾ je, ¾e
struktura hlavièky u formátu .7z je znaènì komplikovaná a promìnná. Ètení struktury ulehèují
odpovídající funkce v této tøídì.

 Tøída mimo jiné obsahuje funkci $SevenZUINT64()$ implementující dekodér 64 bitových
bezznamýnkových èísel. Tohoto kódování velkých èísel je ve formátu .7z pou¾ito k redukování
pamì»ového prostoru potøebného k ulo¾ení takového èísla. Ve zkratce to funguje tak, ¾e poèet
jednièek v nepøeru¹ené øadì zaèínající na nejvy¹¹ích pozicích v prvním bajtu urèuje kolik
následujících bajtù se je¹tì vá¾e k tomuto èíslu. Zbytek prvního bajtu za prvním výskytem nuly je
pou¾it k ulo¾ení hodnoty nejvýznamnìj¹ího bajtu~\cite{Pavlov:2010}. 

\subsection{SevenZCrackerCPU}
\label{ssec:7zcrackercpu}
Tato tøída obsahuje implementaci funkcí potøebných pro ovìøení hesla na CPU. Stejnì tak jako
v¹echny ekvivalentní tøídy v ostatních modulech i tato obsahuje funkci $checkPassword()$ a pak
funkce pro vytvoøení klíèe pro de¹ifrování a funkci se starající se o samotné de¹ifrování a
dekompresi dat.

 Funkce pro de¹ifrování a dekompresi byly pøevzaty z nástroje 7zip vytvoøeného autorem formátu .7z
Igorem Pavlov, tyto funkce vytvoøil pod licencí Public Domain. Díky tomu, ¾e nástroj 7zip je
napsán v C / C++ nebylo tøeba nijak do funkcí respektive tøíd vùbec zasahovat. Funkce $hash()$
vytváøející klíè pro de¹ifrování byla také znaènì inspirována zdrojovými kódy 7zip-u. 

 Ve funkci $checkPassword()$ je pou¾ita rychlostní optimalizace slou¾ící k redukci èasu
potøebného k urèení zda se mù¾e jedna o správné heslo. Optimalizace funguje následujícím zpùsobem:
\begin{enumerate}
    \item Zjistíme jestli jsou data pro de¹ifrování velká alespoò 32 bajtù. Pokud jsou men¹í tak
	se optimalizace nepou¾ije.
    \item Vezmeme prvních 16 bajtù dat.
    \item Provedeme nad nimi operaci de¹ifrování.
    \item Ovìøíme hodnoty na zaèátku de¹ifrovaných dat. Pokud je první bajt roven 0x00
	(pevná první hodnota dat dekomprimovatelnch LZMA) nebo pokud se první tøi bajty rovnají
	0x0104006 (posloupnost signatur v hlavièce archivu typu .7z) tak pokracujeme podle
	Algoritmu~\ref{alg:7z_desifr} pro celé data souboru. Jinak heslo zahodíme a jdeme o
	zaèátku s novým heslem.
\end{enumerate}
\subsection{SevenZCrackerGPU}
Jak ji¾ název naznaèuje jedná se o tøídu s implementací funkce pro inicializaci dat na GPU a s
funkcí pro ovìøení hesla získaného z GPU. Aèkoliv to vypadá velmi podobnì je zde jeden velký
rozdíl v tom, kdy a jak dostaneme správné heslo. S jistou nadsázkou se dá øíct, ¾e se jedná o
tøídu provádìjící obnovení hesla na CPU s jistou formou heuristiky. 

Formout heuristiky jsou kroky 1 - 4 optimalizace popsáné v pøedchozí podkapitole. Rozdíl oproti
CPU implementaci je ten, ¾e filtrování hesel obstarává GPU a nemusíme ho tedy provádìt na CPU.
Teoreticky by nám tímto zpùsobem filtrování sní¾í poèet hesel zhruba 256-krát v pøípadì, ¾e
¹ifrovaná data jsou i komprimovaná data. V pøípadì, ¾e se bavíme archivech .7z, které obsahují
pouze jeden soubor a jejich hlavièka tedy pøed ¹ifrováním není komprimovaná, pak se dostáváme
daleko vìt¹í redukci hesel.

\subsubsection{sevenz\_aes\_kernel}
Kernel obsahuje pøepis tøíd pøevzatých z nástroje 7zip. Jediné modifikace tìchto tøíd jsou
...
Dùvodem proè neprovádíme v¹echny kroky na GPU je pomìrnì jednoduchý. GPU nemá dostatek pamìti na
to, aby mohla obsahovat nìkolik instancí dat k de¹ifrování. V archivu mohou být obsa¾eny soubory o
velikosti nìkolika gigabajtù a není tedy v na¹í moci takto velký shluk dat vmìstnat. Pokusy o
vmìstnání nìkolika instancí takovýchto dat je tedy naprosto nereálné. 

\chapter{Mìøení a srovnání}
