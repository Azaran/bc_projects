...
k~tomu urèeny. 
 ...
 Toho lze dosáhnout ¹ifrováním informací. V~dne¹ní dobì existuje nespoèet mo¾ností jak
informace ¹ifrovat. Kvalitu zabezpeèení dat znaènì ovlivòuje výbìr pou¾itého ¹ifrovacího algoritmu.
Ov¹em zvolení správného hesla má vliv je¹tì vìt¹í. Pokud pou¾ijete slabou ¹ifru a silné heslo, je
¹ance, ¾e se k~Va¹im datùm dostane neoprávnìná osoba je o~dost men¹í, ne¾ kdy¾ to udìláte naopak.
...
zabývá pøiblí¾ením paralelismu a problémù s~ním spojených a také uvedením do standardu a
frameworku OpenCL.
...
\begin{itemize}
    ...
    \item Posledním re¾imem je pou¾ití jednoho 56-bitového klíèe a tedy K1 = K2 = K3.
\end{itemize}
...
bychom chtìli aplikaci provozovat. Proto se vyskytla potøeba vytvoøit univerzální rozhraní, jej¾ by
...
archivù, jej¾ obsahují soubory v~za¹ifrované podobì. Zda je soubor ¹ifrován lze zjistit z~jeho
...
kdy¾ jak u¾ bylo vý¹e øeèeno je tento modul pro Wrathion ji¾ vytvoøen, tak se divíte správnì.
V~pøípadì tohoto modulu se bude zabývat roz¹íøením funkcionality a ne vytvoøením zcela nového
modulu.
...
speciální verzi AES, jej¾ si vytvoøili vývojáøi nástroje WinZip. Ti si vytvoøili vlastní oøezanou
...
neporadí vùbec a tak je tedy vhodné tuto funkcionalitu doplnit. Doplnìním, sní¾íme poèet ZIP
archivù, jen¾ tento modul nezvládne zpracovat.
...
detailní porozumìní struktuøe je tøeba nahlídnout do souboru 7zFormat.txt~\cite{Pavlov:2010}.
Potøebné informace lze získat následujícím znaènì zjednodu¹eným teoretickým postupem:  
\begin{enumerate}
    \item Ovìøíme signaturu -- jedná se o~první 6 bajtù souboru, musí odpovídat hodnotì
v~sekci \ref{ssec:7z_struct}. 
    \item Získáme pozici a velikost dal¹í hlavièky -- pøeskoèíme na pozici 6 bajtù za signaturou a
	získáme pozici dal¹í hlavièky. Z~následující hodnoty získáme délku následující hlavièky.
	...
	4 obdobným zpùsobem, av¹ak nyní pracujeme s~daty o~souborech namísto o~hlavièce.
\end{enumerate}
\begin{algorithm}[H]
    \SetStartEndCondition{ (}{)}{)}\SetAlgoBlockMarkers{}{}%
    \SetKwFor{For}{for}{\string:}{}%
...
ve vìt¹inì pøípadù ani nestojí za námahu. Dùvodem je nízký poètu dostupných nástrojù schopných tyto
